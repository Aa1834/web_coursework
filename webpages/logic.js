...
async function getDifference() {
  ...
  if (initialTime !== null) {
    ...
    const rowData = {position: position, time: formattedTime, dateTime: dateString};
    createTableGridTime([rowData])

    dataToServer.push(rowData);

    localStorage.setItem('dataToServer',JSON.stringify(dataToServer));
    console.log('Saved dataToServer:', localStorage.getItem('dataToServer'));
...
/*async function uploadData() {
  // raceData = object (localStorage is an object) with key-value pairs from localStorage
  const raceData = { ...localStorage }; // Spread operator here expands the object localStorage into key-value pairs; creates new object raceData that contains all the key-value pairs from localStorage.
  console.log(JSON.stringify(raceData)); // convert raceData to JSON string
  ...
  const response = await fetch('/', {
    ...
    body: JSON.stringify(raceData), // convert the raceData to JSON string and then post the localStorage time and position to the server.
  });
} */
...
/*function saveTimes(){ // save the position and race times recorded in table cells in local storage before
  ...
  function loadTableData() {
    const savedData = localStorage.getItem('dataToServer');
    if (savedData) {
      ...
      for (let i = 0; i < parsedData.length; i++) {
        ...
        timeBox.type = 'text';
        timeBox.value = item.time;
        timeBox.addEventListener('change', (event) => {
          item.time = event.target.value; 
          localStorage.setItem('dataToServer', JSON.stringify(parsedData)); 
        });
        timeCell.appendChild(timeBox);
...
function capturePhoto() {
  ...
  context.fillText(`${currentTime}`, 10, canvas.height - (canvas.height - 40));
  ...
  img.alt = `A captured photo: Photo ${photoID}`;
...
const altButton = document.querySelector('#second');
altButton.addEventListener('click', capturePhoto); */
